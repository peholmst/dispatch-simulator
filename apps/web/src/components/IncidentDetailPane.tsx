...
export function IncidentDetailPane({ shift, incident, incidentLocation, code, priority, validPriorities, onCodeChange, onPriorityChange, run }: {
  shift?: ShiftState;
  ...
  const elapsedSince = incident?.reportedAt ?? incident?.createdAt;
      ...
      {incident ? (
        <>
          <div className="incident-detail-summary">
            <div className="incident-status-line">
              <div className={`status ${incident.status}`}>{incident.status.replaceAll("_", " ")}</div>
              <output className="incident-elapsed" aria-label="Incident elapsed time">
